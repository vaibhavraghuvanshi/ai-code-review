...
const queue: Array<{ jobId: string; payload: { code: string; language: string; userId?: string; persist?: boolean } }> = [];
let working = false;
...
router.post("/ai/review/async", rateLimit(20), async (req: Request, res: Response) => {
  ...
  queue.push({ jobId, payload: { code, language: typeof language === "string" ? language : "javascript", userId, persist } });
  // process soon
