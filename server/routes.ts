...
router.patch("/subscriptions/:id/admin", async (req: Request, res: Response) => {
  ...
  const schema = z.object({
    ...
    endDate: z.union([z.string().datetime().transform((s) => new Date(s)), z.null()]).optional(),
  });
  const parsed = schema.safeParse(req.body);
...
router.post("/ai/review", async (req: Request, res: Response) => {
  try {
    const { code, language, persist, model, strictModel } = req.body || {};
  ...
  const result = await reviewCodeWithGroq({
      code,
      language: lang,
      model: typeof model === "string" ? model : undefined,
      strict: Boolean(strictModel),
    });
...
type Job = { id: string; status: "queued"|"processing"|"completed"|"failed"; result?: any; error?: string };
const jobs = new Map<string, Job>();
const queue: Array<{ jobId: string; payload: { code: string; language: string; userId?: string; persist?: boolean; model?: string; strictModel?: boolean } }> = [];
let working = false;
async function processQueue() {
  ...
  while (queue.length) {
    ...
    try {
  const { code, language, userId, persist, model, strictModel } = item.payload;
  const result = await reviewCodeWithGroq({ code, language, model, strict: Boolean(strictModel) });
      // persist if needed
      if (persist !== false) {
        try {
          await storage.createReview({
            ...
            reviewText: result.summary,
            fixedCode: result.fixedCode,
            suggestions: (Array.isArray(result.issues) ? result.issues : []).map((i: any) => `- ${i.message}`).join("\n"),
...
router.post("/ai/review/async", rateLimit(20), async (req: Request, res: Response) => {
  const { code, language, userId, persist, model, strictModel } = req.body || {};
  if (!code || typeof code !== "string") {
    return res.status(400).json({ error: "code is required" });
  }
  const jobId = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  jobs.set(jobId, { id: jobId, status: "queued" });
  queue.push({ jobId, payload: { code, language: typeof language === "string" ? language : "javascript", userId, persist, model: typeof model === "string" ? model : undefined, strictModel: Boolean(strictModel) } as any });
  // process soon
