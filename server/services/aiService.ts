...
- Prefer minimal, safe changes. Avoid stylistic rewrites unless necessary.`;
  ...
  const candidates = Array.from(
    new Set([normalizeModel(GROQ_MODEL), ...GROQ_FALLBACK_MODELS.map(normalizeModel)]),
  );
  ...
  for (const model of candidates) {
    try {
      ...
      if (!res.ok) {
        ...
        throw Object.assign(
          new Error(`Groq API error ${res.status}: ${(txt || '').slice(0, 200)}`),
          { status },
        );
      }
      ...
      const toolArgsStr: string | undefined =
      ...
      // Basic sanitize
      const issues: AiIssue[] = Array.isArray(json.issues)
        ? json.issues.map((i: any, idx: number) => ({
  ...
  throw Object.assign(new Error('Failed to call Groq with any available model'), {
