...
- Prefer minimal, safe changes. Avoid stylistic rewrites unless necessary.`;
  ...
  const candidates = strict && model
    ? [normalizeModel(model)]
    : Array.from(
        new Set([
          ...(model ? [normalizeModel(model)] : []),
          normalizeModel(GROQ_MODEL),
          ...GROQ_FALLBACK_MODELS.map(normalizeModel),
        ])
      );
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if ((res.status === 400 || res.status === 404) &&
          ...
          const err = Object.assign(new Error(`Groq API error ${res.status}: ${(txt || "").slice(0, 200)}`), { status: res.status });
          if (strict) {
            (err as any).attemptedModels = [model].filter(Boolean);
            throw err;
          }
          lastErr = err;
