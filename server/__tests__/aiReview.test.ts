    ...
    mockFetch.mockResolvedValueOnce({
      ...
      json: async () => ({
        choices: [{ message: { content: JSON.stringify({
          summary: "ok",
          fixedCode: "const x=1;",
          issues: [{ id: "1", message: "use const", severity: "info", startLine: 1, startColumn: 1, endLine: 1, endColumn: 10 }]
        }) } }],
        usage: { prompt_tokens: 10, completion_tokens: 20, total_tokens: 30 },
      })
    });

    const res = await request(app).post("/api/ai/review").send({ code: "var x=1;", language: "javascript", persist: false });
